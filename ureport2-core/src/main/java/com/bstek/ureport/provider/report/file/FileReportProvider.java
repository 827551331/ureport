...
public class FileReportProvider implements ReportProvider, ApplicationContextAware {
    private static final Logger logger = LoggerFactory.getLogger(FileReportProvider.class);
    ...
    public void saveReport(String file, String content) {
        ...
        String fullPath = fileStoreDir + "/" + file;
        //2022-08 add 判断路径文件是否存在，不存在则直接创建 wang_yw
        File targetFile = new File(fullPath);
        if (!targetFile.exists()) {
            try {
                if (targetFile.createNewFile()) {
                    logger.info("#创建新文件成功，文件地址：{}", fullPath);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
